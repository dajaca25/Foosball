...
public class AvatarBoneController : BoneController
{
...
*/
    ...
    public void MapBones(ref Transform [] bones, ref string [] boneNames )
    {
        int addCount=0;
        Debug.Log("BoneControllerAvatar:MapBones for "+ this.name);
        for (int boneIndex = 0; boneIndex < bones.Length; boneIndex++)
//        for (int i = 0; i < k_NumSkeletonJoints; i++)
        {

            HumanBodyBones bi = AvatarSkeleton.boneIndex2MecanimMap[boneIndex];
            ...
            string jname = bi.ToString();
            //string toRemove = "per";
            jname = System.Text.RegularExpressions.Regex.Replace(jname, "Spine", "Spine1");
            jname = System.Text.RegularExpressions.Regex.Replace(jname, "UpperLeg", "UpLeg");
            jname = System.Text.RegularExpressions.Regex.Replace(jname, "LowerLeg", "Leg");
            jname = System.Text.RegularExpressions.Regex.Replace(jname, "Shoulder", "Shoulder1");
            jname = System.Text.RegularExpressions.Regex.Replace(jname, "UpperArm", "Arm");
            jname = System.Text.RegularExpressions.Regex.Replace(jname, "LowerArm", "Forearm");
...
//}
            ...
            int index = GetJointIndex(jname);
            if (index<0){
                print(this.name + "'s joint name not found for "+ jname);
                continue;
            }
            //NOTE: BoneController.cs needs m_BoneMapping changed to public
            bones[boneIndex] = m_BoneMapping[index];//animatorComponent.GetBoneTransform(bi);
            boneNames[boneIndex] = bi.ToString();
            Debug.Log("BoneAvatarController:MapBones:bones["+boneIndex+"] = " + bones[boneIndex]+ ",name= "+boneNames[boneIndex]);
            addCount++;
                Debug.Log("BoneAvatarController:MapBones:Adding bone["+boneIndex+"] = " + jname + " for "+ this.name);

        }
        Debug.Log("BoneAvatarController:MapBones: added " + addCount+ " of "+ k_NumSkeletonJoints+ " for " + this.name);
